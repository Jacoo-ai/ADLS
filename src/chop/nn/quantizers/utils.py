...
class BinaryZeroScaled(InplaceFunction):
    ...
    def forward(ctx, input, _threshold):
        ...
        # Fix: Ensure correct alpha shape for broadcasting
        # alpha = alpha.view(input.shape[0], *[1] * (input.dim() - 1))
        alpha = alpha.expand_as(input)

        # Fix: Dynamically expand based on input dimensions
        if input.dim() == 2:
            output = pos_one * alpha.expand(input.shape[0], input.shape[1])
        elif input.dim() == 3:
            output = pos_one * alpha.expand(input.shape[0], input.shape[1], input.shape[2])
        elif input.dim() == 4:
            output = pos_one * alpha.expand(input.shape[0], input.shape[1], input.shape[2], input.shape[3])
        else:
